...
- (void)addSomeViews {
  ...
  cross.center = CGPointMake(self.mainView.bounds.size.width / 2,
                             self.mainView.bounds.size.height / 2);
  cross.bounds =
      CGRectMake(0, 0, menuFrame.size.width / 2, menuFrame.size.width / 2);
  ...
  UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(tapToSwitchOpenOrClose)];
...
- (void)setUpSomeDatas {
  ...
  for (int i = 0; i < menuCount; i++) {
    ...
    CGPoint center = CGPointMake(originPoint.x + distance * cosDegree,
                                 originPoint.y - distance * sinDegree);
    ...
    UIImageView *menuImage = [[UIImageView alloc]
        initWithFrame:CGRectMake(0, 0, imageWidth, imageWidth)];
    menuImage.center =
        CGPointMake(item.frame.size.width / 2, item.frame.size.height / 2);
...
- (CGPathRef)getRightLinePathWithAmount:(CGFloat)amount {
  ...
  CGPoint pointB = CGPointMake(self.mainView.center.x,
                               self.mainView.center.y -
                                   self.mainView.bounds.size.height / 2);
  CGPoint pointC =
      CGPointMake(self.mainView.center.x + self.mainView.bounds.size.width / 2,
                  self.mainView.center.y);
  CGPoint pointP = CGPointMake(
      self.mainView.frame.origin.x + self.mainView.bounds.size.width +
          amount * cosf(45 * (M_PI / 180)),
      self.mainView.frame.origin.y - amount * cosf(45 * (M_PI / 180)));

  [verticalLine moveToPoint:pointC];
  [verticalLine addQuadCurveToPoint:pointB controlPoint:pointP];
  [verticalLine addArcWithCenter:self.mainView.center
                          radius:R
                      startAngle:M_PI_2
                        endAngle:M_PI
                       clockwise:NO];
...
- (CGPathRef)getLeftLinePathWithAmount:(CGFloat)amount {
  ...
  CGPoint pointB = CGPointMake(self.mainView.center.x,
                               self.mainView.center.y -
                                   self.mainView.bounds.size.height / 2);
  CGPoint pointO = CGPointMake(
      self.mainView.frame.origin.x - amount * cosf(45 * (M_PI / 180)),
      self.mainView.frame.origin.y - amount * cosf(45 * (M_PI / 180)));
...
- (void)menuTap:(UITapGestureRecognizer *)tapGes {
  ...
  for (int i = 0; i < menuCount; i++) {
    if ((tapGes.view.tag == i + 1) &&
        [self.menuDelegate respondsToSelector:@selector(menuDidSelected:)]) {
...
- (void)tapToSwitchOpenOrClose {
  ...
  if (verticalLineLayer == nil) {
    ...
    [self.containerView.layer insertSublayer:verticalLineLayer
                                       below:self.mainView.layer];
  }

  if (isOpened == NO) {
    CAKeyframeAnimation *morph_right =
        [CAKeyframeAnimation animationWithKeyPath:@"path"];
    morph_right.timingFunction =
           ...
           usingSpringWithDamping:0.4f
            initialSpringVelocity:0.0
                       ...
                       animations:^{
                         ...
                         NSValue *pointValue = [PointsDic
                             objectForKey:[NSString
                                              stringWithFormat:@"center%d",
                                                               (int)item.tag]];
    ...
    for (UIView *item in Menus) {
      ...
      CAKeyframeAnimation *morph_left =
          [CAKeyframeAnimation animationWithKeyPath:@"path"];
      morph_left.timingFunction =
          [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
...
- (void)animationDidStop:(CAAnimation *)anim finished:(BOOL)flag {
  ...
  } else if (anim == [verticalLineLayer animationForKey:@"bounce_1_0_left"] ||
             [verticalLineLayer animationForKey:@"bounce_0_1_left"]) {
